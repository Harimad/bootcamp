<template>
  <div class="mt-3">
    <div class="row gy-2 gx-3 align-items-center">
      ...
      <div class="col-auto">
        <button class="btn btn-primary" @click="doSearch">검색</button>
        <button
...
export default {
  ...
  methods: {
    ...
    doSave() {
      ...
      if (isEmptyPhone) {
        return this.$swal('입력되지 않은 Phon이 있습니다.', '', 'info')
      }
      if (isEmptyAddress) {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          ...
          this.selectedCustomers.forEach(async (id) => {
            const r = await this.$put(`/customers/${id}`)
            if (r.status !== 200) {
              isSuccess = false
            }
          })
